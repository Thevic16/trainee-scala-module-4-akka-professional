...
class Waiter(coffeeHouse: ActorRef, barista: ActorRef,
             maxComplaintCount: Int) extends Actor {
  ...
  override def receive: Receive = {
    case ServeCoffee(coffee) => coffeeHouse ! CoffeeHouse.ApproveCoffee(coffee = coffee, guest = sender())
    ...
    case Complaint(coffee) =>
      complaintCount += 1
      barista ! Barista.PrepareCoffee(coffee = coffee, guest = sender())
  }
}
